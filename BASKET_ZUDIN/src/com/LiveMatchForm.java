...
public class LiveMatchForm extends JFrame {
    ...
    private void handleSubstitution(Team team, JButton[] playerButtons) {
        if (selectedPlayerButton == null || !Arrays.asList(playerButtons).contains(selectedPlayerButton)) {
            JOptionPane.showMessageDialog(this, "Выберите игрока на площадке для замены.", "Внимание", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        ok.addActionListener(ev -> {
            ...
            if (newP != null) {
                ...
                selectedPlayerButton.setText("<html><center>Игрок " + newP.getGameNumber() + "<br>" + newP.getName() + "</center></html>");
